...
public class ChildrenReader {
    ...
    public static List<Child> read(String fileName){
        ...
        try (
            ...
            while ((line = br.readLine()) != null){
                children.add(createChildFromString(line));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }return children;
    ...
    private static Child createChildFromString(String line) {
        ...
        Smoke[] smokeValues = Smoke.values();

        Sex sex = sexValues[(Integer.parseInt(split[5]) - 1)];
        Smoke smoke = smokeValues[(Integer.parseInt(split[6]) - 1)];
